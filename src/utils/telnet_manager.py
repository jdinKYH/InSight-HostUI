...
class TelnetManager:
    ...
    def connect_and_receive_initial(self, ip: str, port: int = 23) -> str:
        ...
        try:
            if self.connect(ip, port):
                data = self.tn.read_until(b'User:', timeout=3)
                if data:
                    msg = data.decode('utf-8', errors='replace')
                    return msg
            return "[연결 실패]"
    ...
    def login(self, username: str = "admin", password: str = "") -> bool:
        ...
        try:
            ...
            # 2. Password: 프롬프트 대기
            response = self.tn.read_until(b'Password:', timeout=3)
            ...
            # 4. 로그인 성공 확인 (프롬프트나 웰컴 메시지 확인)
            final_response = self.tn.read_very_eager()
            if final_response:
